...
$Install_AdobeReaderPDF_w10 = $urlsData.UtilitiesForWindows[1] # Acessa a URL do pacote
$Install_AdobeReaderPDF_w11 = $urlsData.UtilitiesForWindows[2] # Acessa a URL do pacote
...
if (Test-Path $directory) {
    ...
    if ($osVersion -like "10.*") {
        $downloadUrl = $Install_AdobeReaderPDF_w10
        $downloadPath = "$env:temp\AcroRdrDC2400220759_pt_BR.exe"
    } elseif ($osVersion -like "11.*") {
        $downloadUrl = $Install_AdobeReaderPDF_w11
        $downloadPath = "$env:temp\AcroRdrDC2400220759_pt_BR.exe"
    } else {
